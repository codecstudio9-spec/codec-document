...
export function OnboardingModal({ open, onOpenChange }: Props) {
  ...
  const handlePlanSelect = (type: PlanType) => {
    // Stashed here (as soon as the plan is known) rather than only right
    // before signing in — GoogleSignInButton renders Google's own button,
    // which we can't attach a "just before click" handler to the way the
    // old signInWithGoogle() button allowed.
    localStorage.setItem('codec_pending_plan_type', type);
    setPlanType(type);
    setStep('auth');
  };
                ...
                >
                  ...
                  {magicSent ? (
                    ...
                    <div className="space-y-3">
                      {/* Google — GIS, same-domain (no supabase.co redirect) */}
